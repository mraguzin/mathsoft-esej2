...
potencijom od $2$ trebale biti zamijenjene odgovarajućim bitovnim posmakom u stvarnim implementacijama.%\footnote{To je temelj učinkovitosti
%ovog algoritma; uočimo također da je moguće umjesto pribrajanja u )
...
\begin{definicija}
    ...
    \end{equation*}
\end{definicija}
Očito nije problem dobiti cjelobrojni korijen iz skoro-korijena. Idući teorem govori, slično kao \ref{tm:isqrt1} za neadaptivni slučaj,
...
dalje analizirati.
\begin{algorithm}\label{alg:nsqrt}
    \floatname{algorithm}{Algoritam}
    \caption{$\mathsf{nsqrt}$ algoritam za račun skoro-korijena (rekurzivna verzija)~\cite{mdickpaper}}
    \begin{algorithmic}[1]
    \Require $N \geq 0$
    \If{$n < 4$}
        \State $rezultat \gets 1$
    \Else
        \State $e \gets \lfloor(M.bitova - 3) / 4\rfloor$
...
dijeljenju $4k$, nakon čega dolazi dijeljenje s  $b$.
...
\begin{algorithm}\label{alg:sqrt}
    \floatname{algorithm}{Algoritam}
...
\begin{equation}
    x = (2^{23} + M)2^{E-127},
\end{equation}
pri čemu treba imati na umu da je $P=0$ zbog nužne pozitivnosti $x$-a te da
...
stvarnosti trebalo implementirati isključivo bitovnim posmacima, povrh jednog nužnog zbrajanja).

Metode temeljene na ovakvim trikovima daju jako dobre rezultate aproksimacije, koji postaju tim bolji što bolje odaberemo
određenu korekcijsku konstantu $k$ koju jednostavno možemo dodati na kraj u \eqref{eq:sqrtshift}. Štoviše, detaljnija analiza iz~\cite{lomont} 
 ...
 za \emph{direktan} račun korijen, već za račun njegove \emph{recipročne} vrijednosti.
...
Newton-Raphsona, postiže se relativna greška od $0.18\%$! Algoritam je implementiran u \ref{sec:invsqrt}, a detaljna analiza preciznosti i ovisnosti
